...
import { getLanguageFull, getSettingSsr } from '@/stores/setting/utils'
...
export default class BaseService {
  ...
  static async _getSSR(pageContext: PageContext, url: string, headers?: HeadersInit) {
    ...
    const { lang: localeCookie } = getSettingSsr(pageContext.cookie)
    const locale = pageContext?.locale

    const res = await pageContext.fetch(ENV.API_ENDPOINT + url, {
      headers: {
        Authorization: `bearer ${tokens?.accessToken}`,
        'Accept-Language': getLanguageFull(locale ?? localeCookie),
        ...headers,
      },
    })
