...
import { Card, Input } from '@olufy-frontend/shared/UI'
...
interface ISendContactAdminFormProps {
  ticketId: number
  onSubmit?: (data: FormData) => void
  onCancel?: VoidFunction
}

const SendContactAdminForm: FC<ISendContactAdminFormProps> = ({ ticketId, onSubmit, onCancel }) => {
    ...
    <Card title={i18n._(t`ตอบกลับ`)}>
      <form className={clsx(`pt-4`)} onSubmit={formik.handleSubmit}>
        ...
        <div className={clsx(`mt-4`)}>
          <label>{i18n._(t`เขียนข้อความ`)}</label>
          <Input.TextArea
            id="message"
            name="message"
...
export default SendContactAdminForm
