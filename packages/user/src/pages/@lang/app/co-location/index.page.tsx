  ...
  const columns = useMemo(() => {
    return [
      ...
      },
      {
        ...
        render: (_val, record) => <ViewButton as="a" href={`/app/co-location/${record.id}`} />,
      },
  ...
  return (
    <Fragment>
      <div className={clsx(`flex items-center space-x-4`)}>
        ...
        <h3 className={clsx(`text-header-3`)}>{i18n._(t`Co-Location`)}</h3>
      </div>

      <div className={clsx(`mt-6 flex items-center`, `sm:flex-col sm:items-start sm:space-y-6`)}>
        <Button variant="success" buttonType="icon-text" as="a" href={`/app/co-location/packages`} size="medium">
