...
export const Page = () => {
  ...
  // _Form
  const formik = useFormik<IHostingSetup>({
    initialValues: {
    ...
    onSubmit: (values) => {
      ...
      push(`/app/hosting/packages/${routeParams?.id}/order?${params}`)
    },
  })
  ...
  return (
    <Fragment>
      <BackButton as="a" href={`/app/hosting/packages/${routeParams.id}`} />
        ...
        <Card className={clsx(`h-fit`)}>
          <div>
            ...
            <div className={clsx(`flex items-center justify-between`)}>
              <span className={clsx(`text-body-16`)}>HOSTING JUNIOR</span>
              <span className={clsx(`text-right text-header-5`)}>
                {formatPrice(500)} THB/{i18n._(t`Yearly`)}
              </span>
            </div>
            <div className={clsx(`mt-2 flex items-center space-x-2`)}>
            ...
            <Divider className={clsx(`my-4`)} />
            <div className={clsx(`mt-2`)}>
              <div className={clsx(`text-body-16`)}>โดเมนเนม</div>
            </div>
            <div className={clsx(`mt-2 flex items-center justify-between`)}>
            ...
            <div className={clsx(`text-right text-header-2 text-primary-500`)}>{formatPrice(850)} THB</div>
