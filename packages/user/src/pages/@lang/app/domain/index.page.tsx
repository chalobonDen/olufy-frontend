...
export const Page = () => {
  ...
  // _Query
  const { data } = useQuery(['get-domain-list'], () => {
  ...
  const columns = useMemo(() => {
    return [
      ...
      },
      {
        ...
        render: (_val, record) => <ViewButton as="a" href={`/app/domain/${record.id}`} />,
      },
  ...
  return (
    <Fragment>
      <div className={clsx(`flex items-center space-x-4`)}>
        ...
        <h3 className={clsx(`text-header-3`)}>{i18n._(t`Domain`)}</h3>
      </div>

      <div className={clsx(`mt-6 flex items-center`, `sm:flex-col sm:items-start sm:space-y-6`)}>
        <Button variant="success" buttonType="icon-text" as="a" href={`/app/domain/packages`} size="medium">
      ...
      <Table rowKey={(_, index) => index} className={clsx(`mt-4`)} columns={columns} dataSource={data ? data : []} />
