...
const getIntegerDigits = (numberInput: string): string => parseInt(numberInput.split('.')[0], 10).toString()

const getFractionalDigits = (numberInput: string): string => parseInt(numberInput.split('.')[1], 10).toString()

const hasFractionalDigits = (numberInput: string): boolean => numberInput !== undefined && numberInput != '0'

const isZeroValue = (number: number): boolean => number == 0
const isUnitPosition = (position: number): boolean => position == UNIT_POSITION
const isTenPosition = (position: number): boolean => position % MAX_POSITION == TEN_POSITION
const isMillionsPosition = (position: number): boolean => position >= MAX_POSITION && position % MAX_POSITION == 0
const isLastPosition = (position: number, lengthOfDigits: number): boolean => position + 1 < lengthOfDigits
...
const getBathText = (position: number, number: number, lengthOfDigits: number): string => {
...
const convert = (numberInput: string): string => {
  ...
  const textOutput = numberReverse.split('').reduce((pre: string, number: string, i: number) => {
...
const parseFloatWithPrecision = (number: number, precision = 2): [string, string] => {
...
export const getThaiBaht = (numberInput: number): string => {
