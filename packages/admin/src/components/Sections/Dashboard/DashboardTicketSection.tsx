...
interface IDashboardTicketSectionProps {
  data: ITicket[]
  count: number
}

const DashboardTicketSection: FC<IDashboardTicketSectionProps> = ({ data, count }) => {
      ...
      <SimpleTableCard
        header={
          ...
          >
            ...
            <div>
              <p className={clsx(`text-body-16 font-light`)}>{i18n._(t`ข้อความใหม่จากผู้ใช้งาน`)}</p>
              <h4 className={clsx(`mt-1 text-header-2`)}>{formatNumber({ number: count })}</h4>
              <a href="#" className={clsx(`flex items-center space-x-2 text-primary-400 underline`)}>
                <span className={clsx(`text-body-14 font-light`)}>{i18n._(t`ข้อความจากผู้ใช้`)}</span>
        ...
        table={{
          ...
          columns: [
            ...
            },
            {
              ...
              align: 'center',
              render: (val) => {
                if (val === TicketStatus.CUSTOMER_REPLY) return <Tag variant="error">{i18n._(t`ลูกค้าตอบกลับ`)}</Tag>
                else if (val === TicketStatus.PENDING) return <Tag variant="warning">{i18n._(t`รอตรวจสอบ`)}</Tag>
                else if (val === TicketStatus.ADMIN_REPLY) return <Tag variant="info">{i18n._(t`ตอบแล้ว`)}</Tag>
...
export default DashboardTicketSection
