...
interface IVpsServerTableActionProps {
  data: ISimpleVpsServer
  onRefetch?: VoidFunction
}

const VpsServerTableAction: FC<IVpsServerTableActionProps> = ({ data: { id, name }, onRefetch }) => {
  ...
  // _Mutation
  const { mutate: onDelete, isLoading: isDeleteLoading } = useMutation((id: number) => VpsServerService.delete(id), {
    onError: (err) => {
    ...
    <Fragment>
      <MoreDropdown
        items={
          <Fragment>
            <Menu.Item>
              <a href={`/app/vps-server/${id}`} className={clsx(`dropdown-dialog-item !min-h-0 !w-full`)}>
      ...
      >
        <p>
          <Trans>
            คุณต้องการลบแพ็กเกจ <strong className={clsx(`underline`)}>{name}</strong> ?
          </Trans>
        </p>
      </ConfirmModal>
    </Fragment>
...
export default VpsServerTableAction
