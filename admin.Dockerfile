...
RUN mkdir -p /app/packages/admin/src/locales && \
    cp -r /app/translations/admin/* /app/packages/admin/src/locales/
...
RUN cp -r /app/packages/admin/src/locales/*.po /app/translations/admin/
...
CMD cd packages/admin && yarn run server:prod
